...
DROP TABLE IF EXISTS `black_unit`;

CREATE TABLE `black_unit` (
  `blackunitid` int(11) NOT NULL AUTO_INCREMENT,
  `unitName` varchar(100) DEFAULT NULL,
  `description` varchar(1000) DEFAULT NULL,
  `addTime` datetime DEFAULT NULL,
  `corporation` varchar(40) DEFAULT NULL,
...
) ENGINE=InnoDB AUTO_INCREMENT=10 DEFAULT CHARSET=utf8 COMMENT='黑名单表';
...
) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=utf8 COMMENT='违章记录表';
...
) ENGINE=InnoDB AUTO_INCREMENT=12022 DEFAULT CHARSET=utf8 COMMENT='Ա�����Ա�';

/*Table structure for table `employeeoutdistribute` */

DROP TABLE IF EXISTS `employeeoutdistribute`;
...
CREATE TABLE `employeeoutdistribute` (
  ...
  KEY `departmentIdIndex` (`departmentId`),
  KEY `empOutIdCardIndex` (`empOutIdCard`),
  KEY `empOutexamStatusIndex` (`empOutexamStatus`),
  KEY `haulempidIndex` (`haulempid`),
  KEY `unitidIndex` (`unitid`)
) ENGINE=InnoDB AUTO_INCREMENT=8050 DEFAULT CHARSET=utf8;

/*Table structure for table `emplyin_breakrules` */

DROP TABLE IF EXISTS `emplyin_breakrules`;
...
) ENGINE=InnoDB AUTO_INCREMENT=16 DEFAULT CHARSET=utf8 COMMENT='违章记录表';
...
CREATE TABLE `exam` (
  ...
  PRIMARY KEY (`examId`),
  KEY `paperId` (`paperId`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='����';
...
) ENGINE=InnoDB AUTO_INCREMENT=63772 DEFAULT CHARSET=utf8 COMMENT='��ʷ�Ծ�ѡ���';
...
DROP TABLE IF EXISTS `haulemployeeout`;
...
DROP TABLE IF EXISTS `haulproject`;
...
DROP TABLE IF EXISTS `haulunitproject`;
...
/*Table structure for table `traincontenttype` */

DROP TABLE IF EXISTS `traincontenttype`;

CREATE TABLE `traincontenttype` (
  `typeId` varchar(40) NOT NULL COMMENT '培训内容类别编号',
  `typeName` varchar(40) DEFAULT NULL COMMENT '培训内容类别名称',
  `upId` varchar(40) DEFAULT NULL COMMENT '培训内容类别上级编号',
  `typeDescription` varchar(40) DEFAULT '0' COMMENT '描述(保留字段)',
  PRIMARY KEY (`typeId`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
/*!50003 DROP TRIGGER*//*!50032 IF EXISTS */ /*!50003 `add_unit_after_add_break` */$$
...
/*!50003 DROP TRIGGER*//*!50032 IF EXISTS */ /*!50003 `update_unit_after_update_break` */$$
...
/*!50003 DROP TRIGGER*//*!50032 IF EXISTS */ /*!50003 `minus_unit_after_delete_haulemp` */$$
...
/*!50003 DROP TRIGGER*//*!50032 IF EXISTS */ /*!50003 `minus_empnum_after_delete_employee` */$$
...
/*!50106 DROP EVENT IF EXISTS `eventEmpInBreaksInfo`*/;
...
/*!50106 DROP EVENT IF EXISTS `eventUpdateStatus`*/;
...
/*!50106 DROP EVENT IF EXISTS `queryOlderPer`*/;
...
/*!50003 DROP PROCEDURE IF EXISTS  `clearEmInBreakInfo` */;
...
/*!50003 DROP PROCEDURE IF EXISTS  `clearEmploutBlack` */;
...
/*!50003 DROP PROCEDURE IF EXISTS  `queryOlderPerson` */;
...
/*!50003 DROP PROCEDURE IF EXISTS  `updateDepartmentIds` */;
...
/*!50001 DROP VIEW IF EXISTS `employee_exam_grade` */;
...
/*!50001 DROP VIEW IF EXISTS `employee_out_base_info` */;
...
/*!50001 DROP VIEW IF EXISTS `exam_employeeexam_exampaper` */;
...
/*!50001 DROP VIEW IF EXISTS `online_exam_employee_info` */;
...
/*!50001 DROP VIEW IF EXISTS `overhaul_unit` */;
...
/*!50001 DROP VIEW IF EXISTS `questionbank_questions_department` */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `exam_employeeexam_exampaper` AS select `exam`.`examId` AS `examId`,`exam`.`examName` AS `examName`,`exam`.`paperId` AS `paperId`,`exam`.`startTime` AS `startTime`,`exam`.`endTime` AS `endTime`,`exam`.`traincontent` AS `traincontent`,`exam`.`status` AS `status`,`exam`.`xueshi` AS `xueshi`,`exam`.`employeeName` AS `employeeName`,`exam`.`bigStatus` AS `bigStatus`,`exam`.`departmentId` AS `departmentId`,`exam`.`examType` AS `examType`,`exampaper`.`paperAnswer` AS `paperanswer`,`exampaper`.`description` AS `description`,`exampaper`.`employeeName` AS `createPaperName`,`exampaper`.`level` AS `level`,`exampaper`.`makeTime` AS `makeTime`,`exampaper`.`paperScore` AS `paperScore`,`exampaper`.`title` AS `title`,`exampaper`.`useTimes` AS `useTimes`,count(`employeeexam`.`employeeId`) AS `sumPerson`,sum((case (((timestampdiff(MINUTE,'2018-06-04',`exam`.`startTime`) >= 0) and (`employeeexam`.`grade` = `exampaper`.`paperScore`)) or ((timestampdiff(MINUTE,'2018-06-04',`exam`.`startTime`) < 0) and (`employeeexam`.`grade` >= (`exampaper`.`paperScore` * 0.9)))) when 1 then 1 else 0 end)) AS `countPassPerson`,`employeeexam`.`examMethod` AS `exammethod` from ((`exam` join `exampaper` on((`exam`.`paperId` = `exampaper`.`paperId`))) left join `employeeexam` on((`exam`.`examId` = `employeeexam`.`examId`))) group by `exam`.`examId` */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `online_exam_employee_info` AS select `exam`.`examId` AS `examId`,`exam`.`examName` AS `examName`,`exam`.`startTime` AS `startTime`,`exam`.`endTime` AS `endTime`,`exam`.`status` AS `status`,`exam`.`paperId` AS `paperId`,`exam`.`AnswerTime` AS `AnswerTime`,`exampaper`.`level` AS `level`,`exampaper`.`paperScore` AS `paperScore`,`employeeexam`.`employeeId` AS `employeeId`,`employeeexam`.`employeeName` AS `employeeName`,`employeeexam`.`employeeType` AS `employeeType`,`employeeexam`.`examMethod` AS `examMethod`,`employeeexam`.`grade` AS `grade`,(case (((timestampdiff(MINUTE,'2018-06-04',`exam`.`startTime`) >= 0) and (`employeeexam`.`grade` = `exampaper`.`paperScore`)) or ((timestampdiff(MINUTE,'2018-06-04',`exam`.`startTime`) < 0) and (`employeeexam`.`grade` >= (`exampaper`.`paperScore` * 0.9)))) when 1 then '是' else '否' end) AS `isPass`,`employee_in`.`sex` AS `sex`,`employee_in`.`idCode` AS `idCode`,`onlineexaminfor`.`startTime` AS `startAnswerTime`,`onlineexaminfor`.`endTime` AS `endAnswerTime`,`onlineexaminfor`.`loginTime` AS `loginTime`,`onlineexaminfor`.`examStatus` AS `AnswerStatus` from ((((`exam` join `exampaper` on((`exam`.`paperId` = `exampaper`.`paperId`))) join `employeeexam` on(((`exam`.`examId` = `employeeexam`.`examId`) and (`employeeexam`.`employeeType` = 0) and (`employeeexam`.`examMethod` = '线上')))) join `employee_in` on((`employee_in`.`idCode` = `employeeexam`.`employeeId`))) left join `onlineexaminfor` on(((`onlineexaminfor`.`employeeId` = `employeeexam`.`employeeId`) and (`onlineexaminfor`.`examId` = `employeeexam`.`examId`)))) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `overhaul_unit` AS select `haulinfo`.`bigId` AS `id`,(case when 1 then 1 else 2 end) AS `upid`,`haulinfo`.`bigName` AS `name`,`haulinfo`.`bigCreateDate` AS `bigCreateDate`,`haulinfo`.`bigStatus` AS `bigStatus` from `haulinfo` union select `haulunit`.`unitId` AS `id`,`haulunit`.`bigId` AS `upid`,`unit`.`name` AS `name`,`haulinfo`.`bigCreateDate` AS `bigCreateDate`,`haulinfo`.`bigStatus` AS `bigStatus` from ((`haulunit` join `unit`) join `haulinfo`) where ((`unit`.`unitId` = `haulunit`.`unitId`) and (`haulinfo`.`bigId` = `haulunit`.`bigId`)) order by `bigCreateDate` desc */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `questionbank_questions_department` AS select `questionbank`.`questionBankId` AS `questionBankId`,`questionbank`.`questionBankName` AS `questionBankName`,`questionbank`.`departmentId` AS `departmentId`,`questionbank`.`employeeName` AS `employeeName`,`questionbank`.`createTime` AS `createTime`,`questionbank`.`isUse` AS `isUse`,`questionbank`.`description` AS `description`,`questionbank`.`categoryNameId` AS `categoryNameId`,`dictionary`.`dictionaryName` AS `typeName`,`dictionary`.`upDictionaryId` AS `upTypeId`,count(`questions`.`questionId`) AS `sumQuestions`,sum((case `questions`.`type` when '单选题' then 1 else 0 end)) AS `countSingle`,sum((case `questions`.`type` when '多选题' then 1 else 0 end)) AS `countMultiple`,sum((case `questions`.`type` when '判断题' then 1 else 0 end)) AS `countTrueOrFalse` from ((`questionbank` join `dictionary` on((`dictionary`.`dictionaryId` = `questionbank`.`categoryNameId`))) left join `questions` on((`questionbank`.`questionBankId` = `questions`.`questionBankId`))) group by `questionbank`.`questionBankId` */;
